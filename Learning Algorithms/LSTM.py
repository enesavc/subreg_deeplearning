...
# ## LSTM learning positive and negative sentences in {SP,SL}{2,4,8}
...
class Predictor(chainer.Chain):
    def __init__(self, num_Sigma, dim=100):
        ...
        self.add_link('lstm' , L.LSTM   (      dim, dim, forget_bias_init=1) )
        self.add_link('lin'  , L.Linear (      dim,   2                    ) )
    ...
    def __call__(self, box, label = None, learning = True ):
        ...
        if label is None:
            ...
            t = xp.asarray(label)
            self.acc += ( F.accuracy(y, t).data.get()*len(box), len(box) )
...
for dataset_name in dataset_names:
    for dim in [10, 30, 100]:
        ...
        model = Predictor(len(Sigma)+1, dim=dim)
        #model.to_gpu()
...
for language in ("SL2","SL4","SL8","SP2","SP4","SP8"):
    for datasize in ("1k","10k","100k"):
        for dim in (10,30,100):
            filename = logpath+LSTMName+"_%s_%s_v%d.log"%(language,datasize,dim)
